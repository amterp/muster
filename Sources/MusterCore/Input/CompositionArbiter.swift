...
/// host has to read those two signals and pick exactly one thing to send. Picking *both*
/// is the failure mode: committed text and an encoded keystroke are two renderings of the
/// same press, and sending both makes `hello` arrive as `hheelllloo`.
///
...
public enum CompositionArbiter {
  ...
  /// - Parameters:
  ///   - wasComposing: whether a composition was already in progress before this press.
  ///   - committed: text the input method handed over during this press, if any.
  ///   - stillComposing: whether a composition remains in progress afterwards.
  public static func outcome(wasComposing: Bool, committed: String?, stillComposing: Bool)
    -> Outcome
  {
    // Still composing: the press selected a candidate or extended a preedit.
    if stillComposing { return .sendNothing }
    ...
    if wasComposing, let committed, !committed.isEmpty { return .sendText(committed) }
