...
//! stream back, and typing into it while it is hidden - `muster pane send` to a pane behind
//! another tab is ordinary - still reaches the program.
//!
...
use support::{Press, Typing, answer, assert_ok, until};
...
fn a_hidden_pane_lets_go_of_its_stream_and_takes_it_back() {
    ...
    // Hidden again, and typed into while it is.
    assert_ok(&answer(request::Payload::CreateTab(CreateTab::default())));
    until(
    ...
    Press::new("KeyC", "c").send();
    typing.expect_on_screen("bc", "what was typed into a hidden pane never reached its program");
}
