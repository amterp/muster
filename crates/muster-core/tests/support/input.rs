//! Fakes the input path needs.
//!
//! Part of the contract, not incidental scaffolding: `pane-input.json` states what they do,
//! so a driver in any language builds the same ones. A fake that spelled a keystroke
//! differently would make every case disagree for a reason unrelated to the behavior under
//! test.
